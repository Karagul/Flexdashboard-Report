...
RED2 = "#e41a1c"
AGENCY_BLUE = "#005da9"; AGENCY_BLUE_LIGHT = "#508dd6"
AGENCY_ORANGE = "#f37736"; AGENCY_ORANGE_RGB = "rgba(243, 119, 54, 0.80)"
...
if (Sys.info()["nodename"] == "GOTO3213490") { # The SAM
    set_config(
        use_proxy(Sys.getenv("PROXY_SERVER_NAME"), 
...
if (conf$mode == "production") {
    ...
    aws.s3::save_object("cor.rds", conf$bucket, file = "cor_s3.rds")
    aws.s3::save_object("sig.rds", conf$bucket, file = "sig_s3.rds")
...
signal_dashboard <- function(sigid, pth = "s3") {
    ...
    if (pth == "s3") {
        ...
        fn <- glue("{sigid}.db")
        aws.s3::save_object(ky, conf$bucket, file = fn)
...
signal_dashboard_older <- function(sigid, pth = "s3") {
    ...
    if (pth == "s3") {
        ...
        aws.s3::save_object(fn, conf$bucket, file = fn)
        #data <- readRDS(fn)
        conn <- dbConnect(RSQLite::SQLite(), fn)
        file.remove(fn)
    } else {
