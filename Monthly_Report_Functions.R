...
if (Sys.info()["sysname"] == "Windows") {
    ...
    python_path <- file.path("~", "Anaconda3", "bin", "python")
...
if (Sys.info()["nodename"] %in% c("Larry")) { # update if we need a proxy
...
get_tmc_routes <- function(pth = "Inrix/For_Monthly_Report/") {
...
get_spm_data_aws <- function(start_date, end_date, signals_list, table, TWR_only=TRUE) {
    ...
    if (Sys.info()["nodename"] == "GOTO3213490") { # The SAM
        ...
        conn <- dbConnect(drv, "jdbc:awsathena://athena.us-east-1.amazonaws.com:443/",
                          s3_staging_dir = 's3://{}'.format(ATHENA_BUCKET),
                          user = Sys.getenv("AWS_ACCESS_KEY_ID"),
                          password = Sys.getenv("AWS_SECRET_ACCESS_KEY"),
                          ProxyHost = Sys.getenv("PROXY_SERVERNAME"),
                          ProxyPort = Sys.getenv("PROXY_PORT"),
                          ProxyUID = Sys.getenv("PROXY_USERNAME"),
                          ProxyPWD = Sys.getenv("PROXY_PASSWORD"))
    } else {
    ...
    query <- glue("SELECT * FROM {ATHENADB}.{tolower(table)} {query_where};")
...
get_qs <- function(detection_events) {
	...
	if (!file.exists(fn)) {
        ...
        bind_rows %>% 
        as_tibble() %>% 
        select(Date, SignalID, CallPhase, TimeFromStopBar) %>%
        mutate(SignalID = factor(SignalID),
               CallPhase = factor(CallPhase))
...
get_cor_avg_daily_detector_uptime <- function(avg_daily_detector_uptime, corridors) {
    ...
    cor_daily_all_uptime <- avg_daily_detector_uptime %>% 
        ...
        get_cor_weekly_avg_by_day(corridors, "uptime.all", "all")
