...
def get_split_failures(start_date, end_date, signals_string):
    ...
    cycle_query = """SELECT DISTINCT SignalID, Phase, PhaseStart 
                     ...
                     AND date {}
                     AND SignalID in {}
                     """.format(ATHENADB, between_clause, signals_string)
    ...
    sor = (query_athena(cycle_query, ATHENADB, ATHENA_BUCKET)
            .assign(PhaseStart = lambda x: pd.to_datetime(x.PhaseStart)))

    det = (query_athena(detector_query, ATHENADB, ATHENA_BUCKET)
